...
run: async (message, args) => {
    ...
    if (!duration) return message.channel.send('Veuillez indiquer une durée valide.').then(sent => sent.delete({timeout: 5e3}))
    ...
    message.channel.send(`${member} a été mute pendant ${humanizeDuration(duration, {language: 'fr'})} !`).then(sent => sent.delete({timeout: 5e3}))
    setTimeout(() => {
